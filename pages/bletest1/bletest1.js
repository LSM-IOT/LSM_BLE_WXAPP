    ...
    wx.createBLEConnection({
      ...
      fail: function(err) {
        ...
        if (err.errCode === 10012) {
          var log = that.data.textLog + "连接超时,请重试!" + "\n";
          // app.showModal1("连接超时,请重试!");
        } else if (err.errCode === 10013) {
          log = that.data.textLog + "连接失败,蓝牙地址无效!!" + "\n";
          // app.showModal1("连接失败,蓝牙地址无效!");
        } else {
          log = that.data.textLog + "连接失败,请重试!!" + "\n";
    ...
    wx.getBLEDeviceServices({
      ...
      success: function(res) {
        ...
        for (let i = 0; i < res.services.length; i++) {
          // 修改处  res.services[i].isPrimary && res.services[i]=='6E400001-B5A3-F393-E0A9-E50E24DCCA9E'
          // if (res.services[i].isPrimary && res.services[i].uuid=='6E400001-B5A3-F393-E0A9-E50E24DCCA9E') { //该服务是否为主服务
          if (res.services[i].isPrimary && res.services[i]) { //该服务是否为主服务 
            ...
            wx.navigateTo({
              url: '/pages/bletest/bletest?name=' + encodeURIComponent(that.data.name) + '&deviceId=' + encodeURIComponent(devId) + '&serviceId=' + encodeURIComponent(res.services[i].uuid)
            });
            return
          }
        }
